...
def filtered_accessible(fst: 'FST') -> 'FST':
    ...
    explored = { fst.initial_state }
    stack = deque([fst.initial_state])
    while stack:
        source = stack.pop()
...
def filtered_coaccessible(fst: 'FST') -> 'FST':
    ...
    stack = deque([s for s in fst.final_states])
    coaccessible = {s for s in fst.final_states}
    while stack:
        source = stack.pop()
    ...
    coaccessible.add(fst.initial_state)  # Let's make an exception for the initial
...
def pushed_weights(fst: 'FST') -> 'FST':
    ...
    if residualweight != 0.0:
        ...
        for s in mainscc & fst.final_states: # Add res w to finals in initial SCC
...
def epsilon_removed(fst: 'FST') -> 'FST':
    ...
    for state, ec in eclosures.items():
        for target, cost in ec.items():
            ...
            if target in fst.final_states:
                if state not in fst.final_states:
                    newfst.final_states.add(mapping[state])
...
def labelled_states_topology(fst: 'FST', mode = 'BFS') -> 'FST':
    ...
    Q = deque([fst.initial_state])
    inqueue = {fst.initial_state}
    while Q:
...
def words_cheapest(fst: 'FST'):
    ...
    cntr = itertools.count()
    Q = [(0.0, next(cntr), fst.initial_state, [])]
    while Q:
...
def determinized(fst: 'FST', staterep = lambda s, w: (s, w), oplus = min) -> 'FST':
    ...
    firststate = frozenset({staterep(fst.initial_state, 0.0)})
    statesets = {firststate:newfst.initial_state}
    if fst.initial_state in fst.final_states:
        newfst.final_states = {newfst.initial_state}
        newfst.initial_state.finalweight = fst.initial_state.finalweight
    ...
    while Q:
        ...
        for label, tset in collectlabels.items():
            ...
            if any(t.targetstate in fst.final_states for _, t in tset):
                newfst.final_states.add(newstate)
                # State was final, so we discharge the maximum debt we can
                newstate.finalweight = oplus(t.targetstate.finalweight + t.weight + \
                    residuals[s] - wprime for s, t in tset if t.targetstate in fst.final_states)
    return newfst
...
def minimized(fst: 'FST') -> 'FST':
    ...
    finalset, nonfinalset = fst.final_states.copy(), fst.states - fst.final_states
...
def merging_equivalent_states(fst: 'FST', equivalenceclasses: set) -> 'FST':
    ...
    newfst.initial_state = statemap[eqmap[fst.initial_state]]
    ...
    newfst.final_states = {statemap[s] for s in fst.final_states if s in representerstates}
    for s in fst.final_states:
        if s in representerstates:
...
def kleene_closure(fst: 'FST', mode = 'star') -> 'FST':
    ...
    for lbl, t in fst.initial_state.all_transitions():
        newfst.initial_state.add_transition(q1[t.targetstate], lbl, t.weight)
    ...
    newfst.final_states = {q1[s] for s in fst.final_states}
    if mode != 'plus' or fst.initial_state in fst.final_states:
        newfst.final_states |= {newfst.initial_state}
        newfst.initial_state.finalweight = 0.0
    newfst.states = set(q1.values()) | {newfst.initial_state}
    return newfst
...
def optional(fst: 'FST') -> 'FST':
    ...
    fst.initial_state = newinitial
    fst.states.add(newinitial)
    fst.final_states.add(newinitial)
...
def concatenate(fst1: 'FST', fst2: 'FST') -> 'FST':
    ...
    for s in fst1.final_states:
        for lbl2, t2 in ocopy.initial_state.all_transitions():
            q1q2[s].add_transition(q1q2[t2.targetstate], lbl2, t2.weight + s.finalweight)

    newfst = FST()
    newfst.initial_state = q1q2[fst1.initial_state]
    newfst.final_states = {q1q2[f] for f in ocopy.final_states}
    for s in ocopy.final_states:
        q1q2[s].finalweight = s.finalweight
    if ocopy.initial_state in ocopy.final_states:
        newfst.final_states |= {q1q2[f] for f in fst1.final_states}
        for f in fst1.final_states:
            q1q2[f].finalweight = f.finalweight + ocopy.initial_state.finalweight
...
def compose(fst1: 'FST', fst2: 'FST') -> 'FST':
    ...
    newfst = FST()
    Q = deque([(fst1.initial_state, fst2.initial_state, 0)])
    S = {(fst1.initial_state, fst2.initial_state, 0): newfst.initial_state}
    while Q:
        A, B, mode = Q.pop()
        currentstate = S[(A, B, mode)]
        currentstate.name = "({},{},{})".format(A.name, B.name, mode)
        if A in fst1.final_states and B in fst2.final_states:
            newfst.final_states.add(currentstate)
...
def reversed(fst: 'FST') -> 'FST':
    ...
    newfst = FST(alphabet = fst.alphabet.copy())
    newfst.initial_state = State()
    mapping = {k:State() for k in fst.states}
    newfst.states = set(mapping.values()) | {newfst.initial_state}
    newfst.final_states = {mapping[fst.initial_state]}
    if fst.initial_state in fst.final_states:
        newfst.final_states.add(newfst.initial_state)
        newfst.initial_state.finalweight = fst.initial_state.finalweight
    mapping[fst.initial_state].finalweight = 0.0

    for s, lbl, t in fst.all_transitions(fst.states):
        mapping[t.targetstate].add_transition(mapping[s], lbl, t.weight)
        if t.targetstate in fst.final_states:
            newfst.initial_state.add_transition(mapping[s], lbl, t.weight + \
...
def reversed_e(fst: 'FST') -> 'FST':
    ...
    newfst = FST(alphabet = fst.alphabet.copy())
    newfst.initial_state = State(name = tuple(k.name for k in fst.final_states))
    mapping = {k:State(name = k.name) for k in fst.states}
    for t in fst.final_states:
        newfst.initial_state.add_transition(mapping[t], ('',), t.finalweight)
...
def union(fst1: 'FST', fst2: 'FST') -> 'FST':
    ...
    for lbl, t in itertools.chain(fst1.initial_state.all_transitions(), fst2.initial_state.all_transitions()):
        newfst.initial_state.add_transition(mapping[t.targetstate], lbl, t.weight)
    ...
    for s in fst1.final_states | fst2.final_states:
        newfst.final_states.add(mapping[s])
        mapping[s].finalweight = s.finalweight
    # If either initial state was final, make new initial final w/ weight min(f1w, f2w)
    newfst.final_states = {mapping[s] for s in fst1.final_states|fst2.final_states}
    if fst1.initial_state in fst1.final_states or fst2.initial_state in fst2.final_states:
        newfst.final_states.add(newfst.initial_state)
        newfst.initial_state.finalweight = min(fst1.initial_state.finalweight, fst2.initial_state.finalweight)
...
def product(fst1: 'FST', fst2: 'FST', finalf = any, oplus = min, pathfollow = lambda x,y: x|y) -> 'FST':
    """Generates the product FST from fst1, fst2. The helper functions by default
       produce fst1|fst2."""
    newfst = FST()
    Q = deque([(fst1.initial_state, fst2.initial_state)])
    S = {(fst1.initial_state, fst2.initial_state): newfst.initial_state}
    ...
    while Q:
        ...
        if finalf((t1s in fst1.final_states, t2s in fst2.final_states)):
            newfst.final_states.add(currentstate)
